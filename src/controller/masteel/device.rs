use crate::model::{ResponseData, TblWarning, TblWarningQuery};
...
pub async fn get_device_list(
    ...
    let default_dt = FastDateTime(fastdate::DateTime::from_str("1999-01-01 00:00:00").unwrap());
...
#[post("/posttest")]
async fn post_device_test(q: web::Json<TblWarningQuery>, req: HttpRequest) -> HttpResponse {
    println!("model: {q:?}");
    HttpResponse::Ok().json(q.0)
}

#[get("/gettest")]
async fn get_device_test(q: web::Query<TblWarningQuery>, req: HttpRequest) -> HttpResponse {
    let qi = q.into_inner();
    info!("{req:?}");
    info!("query: {qi:?}");
    HttpResponse::Ok()
